...
void MTIMER1_voidInit(void)
{
	...
	/* for OC1A */
	CLR_BIT(TCCR1A,6);
	SET_BIT(TCCR1A,7); // clear OC1A @ compare match and set OC1A @ bottom
	/* for OC1B */
	CLR_BIT(TCCR1A,4);
	SET_BIT(TCCR1A,5); // clear OC1A @ compare match and set OC1A @ bottom
	...
	#endif	
	/* set OCR1A and OCR1B with values if needed  */
	//OCR1A= ;
	//OCR1B= ;
